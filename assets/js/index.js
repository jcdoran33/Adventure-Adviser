...
function updateEventList (data) {
  ...
  for (var i = 0 ; i < 5; i++) {
    ...
    $("#events").children("ul").children("li").eq(i).text(data._embedded.events[i].name + " || " + data._embedded.events[i].dates.start.localDate + " || Start time: " + data._embedded.events[i].dates.start.localTime + " || Ticket Link:")
